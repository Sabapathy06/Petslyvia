...
export function GoogleAuthModal({ isOpen, onClose, onSuccess }: GoogleAuthModalProps) {
  const { loginWithGoogleAccount, loginWithGoogle } = useAuth();
  ...
  const handleBrowserOAuth = async () => {
    ...
    if (res.success) {
      ...
      setError(res.error || 'Failed to initialize Google account selector.');
    }
  };
          ...
          <div className="p-6 space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 bg-[#f4f8f5] hover:bg-[#eaf2ec] text-[#1b382b] font-bold text-xs rounded-xl border border-[#e2ece5] transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                ...
                Sign In using Browser Google Account Chooser
              </button>
            </form>
          </div>
        </motion.div>
